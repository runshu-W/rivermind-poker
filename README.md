...
- GGPoker 现金桌解析器：共用同一份行语法，明确拒绝跑两次/三次、EV Cashout、Cash Drop、赏金、非德州与锦标赛，而不是勉强解析；**尚未用真实导出验证**；
- 手牌边界前缀由解析器自己声明，新增站点无需改动导入器；
...
- 296 项自动化测试、黄金集清单，以及含 1,000 节点匹配与索引化匹配、制品验证、单节点查询和质量门耗时的可重复性能基准；
- Beta 范围与架构文档。
