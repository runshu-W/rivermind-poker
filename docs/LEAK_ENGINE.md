...
AI 教练通过 `ExplanationEvidence` 读取脱敏后的结构化 `LeakCard`，把固定数值和证据组织成自然语言；它不能改变状态、严重度、比例、区间、阈值或证据手牌。候选模型输出必须通过证据哈希、字段引用、数值、隐私和行动指令校验，失败时回退到确定性模板。完整协议见 [AI_COACH.md](AI_COACH.md)。
