...
**守卫检查的是「仓库里有什么」，不是「磁盘上有什么」。** 文件集合来自 `git ls-files --cached --others --exclude-standard`：已跟踪的（含被 `-f` 强加进来、绕过 `.gitignore` 的），加上未跟踪但也没被忽略的（下一次 `git add -A` 就会扫进去）。被忽略的文件不算。

这条是踩出来的：守卫最初用 `rglob` 走整个工作树，函数却叫 `_tracked_files`——名字声称在问 git，实际没问。结果任何跑过一次程序的开发机上都有 `data/private/*.db`，那个文件正是因为**永远不该被提交**才写进 `.gitignore` 的，守卫却对它报错。这不是严格，是错；而且**一个永远会响的检查，人会学会无视它**——这句话就写在同一个文件里，用来解释密钥启发式为什么要跳过占位符。

git 不可用或当前目录不是仓库时，回退到走文件系统。回退比 git 更严（会看见被忽略的文件），所以是失败关闭，但输出会说明用的是哪一种，免得一次「通过」含义不明。
