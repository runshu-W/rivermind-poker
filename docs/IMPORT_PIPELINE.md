...
2026-08-18 在当前开发环境运行合成 10,000 手基准：导入并同步物化 20,000 个玩家统计行用时 2.980 秒，约 3,356 手/秒；随后从宽表聚合 Hero 统计用时 0.188 秒，约 53,179 手/秒；SQLite 文件约 53.57 MB。输入是已提交黄金牌谱的不同手牌 ID 变体，只用于发现性能回退，不代表真实牌谱覆盖率。
