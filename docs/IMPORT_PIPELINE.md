...
2026-08-18 在当前开发环境运行合成 10,000 手基准：导入并同步物化统计与结果表用时 3.748 秒，约 2,668 手/秒；统计聚合 0.183 秒，约 54,717 手/秒；Session 聚合 0.098 秒；筛选最近 1,000 手 CBet 牌谱 0.067 秒；包含证据查询的漏洞评估 0.227 秒；SQLite 文件约 56.78 MB。输入是已提交黄金牌谱的不同手牌 ID 变体，只用于发现性能回退，不代表真实牌谱覆盖率。
