...
- `player_hand_stats`：每位玩家每手一行的维度与确定性指标，和手牌在同一事务写入。
- `player_hand_results`：每位玩家的投入、返还、收池、净结果、时间和账本校验状态。

`UNIQUE(site, hand_id)` 和唯一指纹共同保证幂等导入。解析器遇到未知玩家动作会显式失败，避免静默丢动作后污染统计。

## 当前基准与限制

2026-08-18 在当前开发环境运行合成 10,000 手基准：导入并同步物化统计与结果表用时 3.863 秒，约 2,589 手/秒；统计聚合 0.185 秒，约 54,115 手/秒；Session 聚合 0.097 秒；筛选最近 1,000 手 CBet 牌谱 0.066 秒；包含证据查询的漏洞评估 0.223 秒；确定性教练模板生成不足 0.001 秒；50 例教练候选评测门用时 0.008 秒；SQLite 文件约 56.78 MB。输入是已提交黄金牌谱的不同手牌 ID 变体，只用于发现性能回退，不代表真实牌谱覆盖率。
