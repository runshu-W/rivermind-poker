...
2026-08-18 在当前开发环境运行合成 10,000 手基准：导入并同步物化统计与结果表用时 3.755 秒，约 2,663 手/秒；统计聚合 0.184 秒，约 54,475 手/秒；Session 聚合 0.096 秒；筛选最近 1,000 手 CBet 牌谱 0.065 秒；包含证据查询的漏洞评估 0.221 秒；确定性教练模板生成不足 0.001 秒；SQLite 文件约 56.78 MB。输入是已提交黄金牌谱的不同手牌 ID 变体，只用于发现性能回退，不代表真实牌谱覆盖率。
